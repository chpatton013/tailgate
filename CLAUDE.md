# CLAUDE.md — working in the tailgate repo
...
| 2 | Transparent L3 routing + host-wide MagicDNS via a Vagrant+QEMU gateway VM | 🔬 probing viability |
...
7. **All-networks proxy vs. L3 routing.** An SSE/app-proxy configured to "include all
   networks" may intercept host traffic to the tailnet CGNAT range (`100.64.0.0/10`).
   Loopback-based approaches (Tier 1: host apps → `127.0.0.1:1055`) sidestep it entirely;
   transparent L3 routing (Tier 2) is exactly the case that may be captured — which is
   what the `tier2/` probe exists to settle before investing in the full gateway.
